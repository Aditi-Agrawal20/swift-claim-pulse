              ...
              <AnimatePresence mode="wait">
                {isScanning ? (
                  <motion.div key="scanning" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    className="h-48 rounded-lg bg-muted relative overflow-hidden flex items-center justify-center">
                    <div className="absolute left-0 right-0 h-0.5 bg-primary/60" style={{ animation: 'scan-line 1s ease-in-out infinite' }} />
                  ...
                  <motion.div key="upload" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    onClick={simulateOCR}
                    className="h-48 rounded-lg border-2 border-dashed border-border hover:border-primary/40 cursor-pointer flex items-center justify-center transition-all group">
                    <div className="text-center">
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between">
              ...
              <span className={`text-sm ${hasReceipt ? 'text-success' : 'text-muted-foreground'}`}>{hasReceipt ? '✓ Scanned' : 'Not uploaded'}</span>
            </div>
            {amount && currency !== 'INR' && (
              <>
                <div className="h-px bg-border" />
